const version = "3.12.12"; //версия программы
const fps = 30; //количество кадров в игровой секунде
const lands = [ //массив цветов ландшафтов
  "#000000", //без ландшафта [0]
  "#80a000", //отравленная зона [1]
  "#00a0a0", //санитарная зона [2]
  "#a000a0", //зона биологической опасности [3]
  "#90a000", //пляжная зона [4]
  "#a00000", //зона повышенного заражения [5]
  "#606000", //свалочная зона [6]
  "#f0a070", //аллергенная зона [7]
  "#a00050", //охотничья зона [8]
  "#0040a0", //морская зона [9]
  "#802000", //взрывоопасная зона [10]
  "#408020", //лагерьная зона [11]
  "#000000", //строительная зона [12]
  "#5000a0", //магическая зона [13]
  "#a05000", //зона строгого контроля [14]
  "#a07800", //человечья зона [15]
  "#0060a0", //научная зона [16]
  "#60c0d0", //леденая зона [17]
  "#50a000", //драконья зона [18]
  "#f0f080", //светлая зона [19]
  "#600000", //военная зона [20]
  "#804080", //таинственная зона [21]
...
function clear() { //метод очистки холста
  ...
  ctx.shadowColor = colors.shadow ?? "#000000";
}
...
event.forest = function(e) {
  if (e.count) {
    vib(50);
    let px = options.size/landscape.res;
    for (let x = 0; x < landscape.res; x++) {
      for (let y = 0; y < landscape.res; y++) {
        if (landscape.type[x][y] == 23) {
          for (let i = 0; i < Math.floor(landscape.pow[x][y]*e.count); i++) arr.push(new Rat(arr.length, x*px+random(px), y*px+random(px)));
        }
      }
    }
  }
